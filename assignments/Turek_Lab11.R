...
{
  y = (y_int + slope * x) + rnorm(length(x), mean = 0, sd = st_dev)
  return (y)
}
...
#plot simulated data using model parameters
plot(
  x = birdhab$ls, 
  y = linear_simulator(
    x = birdhab$ls,
...
#plot simulated data with original data
plot(
...
#simulating effects sizes (change slope)
alpha = 0.05
n_sims = 1000
p_vals = numeric(n_sims)
...
#simulating sample sizes (changes length of x)
alpha = 0.05
n_sims = 1000
p_vals = numeric(n_sims)
...
#simulating Effect size (slope) AND sample size (length x)
alpha = 0.01
n_sims = 50
...
require(rgl)
#3D interactive plot with perp3d
persp3d(
  x = sim_n_effect_size$effect_size,
  y = sim_n_effect_size$sample_size,
  z = sim_n_effect_size$power,
...
#save simulations to text file to load later (wont have to re-run)
save(
  sim_n_effect_size,
...
#create our own simulation varying dispersion (pop sd)
require(here)
...
for(j in 1:length(pop_sds))
{
  ...
  for(i in 1:n_sims)
  {
    fit_sim = linear_sim_fit(x = birdhab$ls, y_int = int_obs, slope = slope_obs, st_dev = pop_sd_j)
    p_vals[i] = summary(fit_sim)$coefficients[2, 'Pr(>|t|)']
  }
  pop_sd_power[j] = sum(p_vals < alpha) / n_sims
}

sim_output_dispersion = data.frame(power = pop_sd_power, sd = pop_sds)
  
# You should save your simulation results so you don't have to run it every time.
save(sim_output_dispersion, file = here("data", "lab_ll_dat_dispersion_sim.RData"))

# Line plot of standard deviation (x-axis) and statistical power (y-axis)
plot(
  power ~ sd, data = sim_output_dispersion,
  type = 'l', xlab = 'Standard Deviation', ylab = 'Statistical Power')
abline(v = sd_obs, lty = 2, col = 'red')


#1111111111111111111111111111111111111111111111111111111111111#
#simulate dispersion and sample size analysis
alpha = 0.05
...
saveWidget(
  rglwidget(),
  file = here("images","sd_samplesize_power_sim_plot.html"),
  selfcontained = TRUE
)
